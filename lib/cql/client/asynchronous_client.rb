    ...
    class AsynchronousClient < Client
      ...
      def execute(cql, *args)
        with_failure_handler do
          ...
          execute_request(Protocol::QueryRequest.new(cql, args, options[:type_hints], options[:consistency], options[:serial_consistency], options[:trace], options[:result_page_size], options[:paging_state]), options[:timeout])
        end
      end
